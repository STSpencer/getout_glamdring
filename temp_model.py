...
def keras_fmin_fnct(space):
    ...
    with strategy.scope():
        ...
        model.add(ConvLSTM2D(filters=space['filters_3'], kernel_size=space['kernel_size_3'],
                             padding='same', return_sequences=True,dropout=space['l2_9']))
        model.add(BatchNormalization())
        
        if space['l2_10']=='l5':
            model.add(ConvLSTM2D(filters=space['filters_4'], kernel_size=space['kernel_size_4'],
                                 padding='same', return_sequences=True,dropout=space['l2_11']))
            model.add(BatchNormalization())
            
        if space['l2_12']=='l6':
            model.add(ConvLSTM2D(filters=space['filters_5'], kernel_size=space['kernel_size_5'],
                                 padding='same', return_sequences=True,dropout=space['l2_13']))
            model.add(BatchNormalization())
...
def get_space():
    return {
        ...
        'l2_14': hp.choice('l2_14', ['l7','no']),
        'Dense': hp.choice('Dense', [10,50,100,200]),
        'l2_15': hp.uniform('l2_15', 0,1),
        'l2_16': hp.choice('l2_16', ['l8','no']),
        'Dense_1': hp.choice('Dense_1', [10,50,100,200]),
        'l2_17': hp.uniform('l2_17', 0,1),
        'l2_18': hp.choice('l2_18', ['l9','no']),
        'Dense_2': hp.choice('Dense_2', [10,50,100,200]),
